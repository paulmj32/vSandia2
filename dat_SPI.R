...
county_map_spi = county_map_spi_l %>%
  ...
  bind_cols(spi_df)
county_proj_spi = county_map_proj %>%
  left_join(county_map_spi, by = c("GEOID","NAME"))
#save(county_proj_spi, file = "county_proj_spi.Rda")

# gg2 = ggplot(county_proj_spi %>% filter(Date == "2014-01-01"))+
#   geom_sf(aes(fill = spi24), color = NA) +
